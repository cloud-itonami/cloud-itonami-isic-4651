(ns techtrade.operation
  "OperationActor -- one computer-and-software-wholesale operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (TechTradeAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Tech Export Governor (:govern)
  ...
  One graph run = one computer-and-software-wholesale operation (intake
  -> advise -> govern -> decide -> commit | hold | approval). No
  unbounded inner loop -- each operation is auditable and checkpointed.
  A tech-order's life is advanced by MANY ops (intake / classification-
  verify / dispatch OR release / settle), each its own independent run.
  ...
  (or :rejected). `:delivery/dispatch`/`:technology/release`/`:invoice/
  settle` ALWAYS reach this node when the governor is clean -- see
  `techtrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [techtrade.techtradeadvisor :as techtradeadvisor]
...
(defn build
  ...
  `techtrade.store/Store`).
  opts:
    :advisor      -- a `techtrade.techtradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (techtradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TechTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (techtradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(techtradeadvisor/trace request p)]})))

      ;; Tech Export Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
